...
## Query-diversity update: Repeat@K retrieval penalty

Added a per-taxonomy-node retrieval-overlap penalty so TTEA no longer rewards queries that repeatedly retrieve the same top-k documents.

Implementation:

- Added an optional pipeline hook: `attack.observe_retrieved_docs(retrieved_docs)`.
- TTEA implements that hook and stores the latest retrieved docs before `parse_response`.
- Each `TaxonomyNode` now maintains `retrieved_doc_ids` and `retrieved_doc_embs`.
- For each response update, TTEA computes `reward = novel_reward - repeat_penalty_mu * Repeat@K`.
- If retrieved documents expose stable ids through metadata such as `index`, Repeat@K uses exact id overlap.
- If no ids are available, TTEA falls back to chunk embedding overlap with `--ak_repeat_doc_sim_threshold`.
- Added CLI args: `--ak_repeat_penalty_mu`, `--ak_repeat_doc_sim_threshold`, and `--ak_repeat_history_limit`.
- Logs now report `novel_reward`, `repeat_at_k`, `repeat_penalty`, and per-node retrieval history sizes.
