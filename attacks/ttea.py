...
class TaxonomyNode:
    ...
    retrieved_doc_embs: List[np.ndarray] = field(default_factory=list)
...
class TTEA(KnowExAttack):
    ...
    def __init__(self, args):
        ...
        self.repeat_penalty_mu = args.repeat_penalty_mu
        self.repeat_doc_sim_threshold = args.repeat_doc_sim_threshold
        self.repeat_history_limit = args.repeat_history_limit
    ...
    def observe_retrieved_docs(self, retrieved_docs):
        self.current_retrieved_docs = retrieved_docs or []
    ...
    def parse_response(self, response):
        ...
        repeat_at_k = self._retrieval_repeat_at_k(node, self.current_retrieved_docs)
        repeat_penalty = self.repeat_penalty_mu * repeat_at_k
        ...
        self._remember_retrieved_docs(node, self.current_retrieved_docs)

        logging.info(
            "TTEA response update: node=%s label='%s' mode=%s shift=%.4f reward=%.4f novel_reward=%.4f repeat_at_k=%.4f repeat_penalty=%.4f parsed_chunks=%d new_chunks=%d anchors_added=%d status=%s posterior=%.4f retrieval_history_ids=%d retrieval_history_embs=%d",
            node.node_id,
            node.label,
            self.current_mode,
            shift,
            ...
            node.status,
            node.posterior,
            len(node.retrieved_doc_ids),
            len(node.retrieved_doc_embs),
        )
        if anchors:
    ...
    def _retrieval_repeat_at_k(self, node: TaxonomyNode, retrieved_docs) -> float:
        if not retrieved_docs:
            return 0.0

        doc_ids, texts = self._retrieved_doc_keys_and_texts(retrieved_docs)
        k = max(1, len(texts))
        if doc_ids and node.retrieved_doc_ids:
            repeats = sum(1 for doc_id in doc_ids if doc_id in node.retrieved_doc_ids)
            return repeats / k

        if not node.retrieved_doc_embs:
            return 0.0

        repeats = 0
        for text in texts:
            emb = np.array(self.attack_emb._embed(text))
            max_sim = max((cos_sim(emb, old_emb) for old_emb in node.retrieved_doc_embs), default=0.0)
            if max_sim > self.repeat_doc_sim_threshold:
                repeats += 1
        return repeats / k

    def _remember_retrieved_docs(self, node: TaxonomyNode, retrieved_docs):
        if not retrieved_docs:
            return

        doc_ids, texts = self._retrieved_doc_keys_and_texts(retrieved_docs)
        for doc_id in doc_ids:
            node.retrieved_doc_ids.add(doc_id)

        if len(node.retrieved_doc_embs) < self.repeat_history_limit:
            remaining = self.repeat_history_limit - len(node.retrieved_doc_embs)
            for text in texts[:remaining]:
                node.retrieved_doc_embs.append(np.array(self.attack_emb._embed(text)))

    def _retrieved_doc_keys_and_texts(self, retrieved_docs) -> Tuple[List[str], List[str]]:
        doc_ids = []
        texts = []
        for rank, item in enumerate(retrieved_docs):
            doc = item[0] if isinstance(item, (tuple, list)) and item else item
            text = getattr(doc, "page_content", "")
            metadata = getattr(doc, "metadata", {}) or {}
            raw_id = metadata.get("index") or metadata.get("id") or metadata.get("source")
